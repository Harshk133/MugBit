...
while True:
    ...
    if event == "Create Document":
        ...
        downloader.download(ImgSub, limit=1, output_dir='dataset', adult_filter_off=True, force_replace=False, timeout=60)
        ...
        path = os.getcwd()
        folders = next(os.walk(path))[1]
        # folders_walk = os.walk(path+'\\'+folders)
        # first = next(folders_walk)[2][0]
        
        if ImgSub in folders:
            input_folder_path = os.path.join(path, ImgSub)
            files = next(os.walk(input_folder_path))[2]

            if files:
                first_image_path = os.path.join(input_folder_path, files[0])
                # imgPath = first_image_path
        ...
        # imgPath = first_image_path
        imgPath = "D:/MyPrograms/Micro Project Versions/Latest Version/Python/dataset/mahalakshumi/Image_1.jpg"
        # print(imgPath)

        # Image Handling
        # image1 = InlineImage(doc1, imgPath[0], width=Mm(20))
        # image2 = InlineImage(doc1, imgPath[1], width=Mm(20))
        image1 = InlineImage(doc1, imgPath, width=Mm(20))
